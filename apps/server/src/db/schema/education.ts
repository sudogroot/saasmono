import { pgTable, text, timestamp, boolean, integer, uuid, index, pgEnum } from "drizzle-orm/pg-core";
...
export const institutionLevelEnum = pgEnum('institution_level_ENUM', ['JARDIN', 'PRIMAIRE', 'COLLEGE', 'SECONDAIRE', 'SUPERIEUR']);
...
export const educationLevel = pgTable("education_level", {
  id: uuid("id").primaryKey().defaultRandom(),
  institutionLevelId: uuid("institution_level_id").notNull().references(() => institutionLevel.id),
...
export const educationLevelSubject = pgTable("education_level_subject", {
  id: uuid("id").primaryKey().defaultRandom(),
