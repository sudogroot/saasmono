  ...
  await testDb.transaction(async (tx) => {
    ...
    if (userIds.length > 0) {
      await tx
        ...
        .where(inArray(authSchema.session.userId, userIds));
    }
    ...
    if (userIds.length > 0) {
      await tx
        ...
        .where(inArray(authSchema.account.userId, userIds));
    }
    ...
    if (userIds.length > 0) {
      await tx
        ...
        .where(inArray(authSchema.verification.identifier, userIds));
    }
