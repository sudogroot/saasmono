...
export function TrialForm({
  ...
  const initialDate = initialData?.trialDateTime
    ? new Date(initialData.trialDateTime).toISOString().split('T')[0]
    : "";
  const initialTime = initialData?.trialDateTime
    ? new Date(initialData.trialDateTime).toTimeString().slice(0, 5)
    : "";
  ...
  const updateMutation = useMutation({
    ...orpc.trials.updateTrial.mutationOptions({
      onSuccess: (data) => {
        ...
        queryClient.invalidateQueries({ queryKey: orpc.trials.listTrials.key() });
        queryClient.invalidateQueries({ queryKey: orpc.trials.getTrialById.key({ trialId: trialId! }) });
        onSuccess?.(data);
      },
      onError: (error: any) => {
        toast.error(`حدث خطأ: ${error.message}`);
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                {isEditing && (
                  <p className="text-xs text-muted-foreground">لا يمكن تعديل رقم الجلسة</p>
                )}
                <FormMessage />
              </FormItem>
            )}
